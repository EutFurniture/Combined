...
app.get('/checkproduct', (req, res) => {
  ...
  db.query(
    ...
    [id, custid], (err, result) => {
      console.log(result.length);
      
      if (result.length === 0) {
        db.query("INSERT INTO cart(customer_id, product_id,totalprice, quantity,date) VALUES (?,?,?,1,NOW());", [custid,id, req.query.price], (err, rest) => {
          res.send(rest);
          console.log(id)
          console.log(custid)
...
app.get('/ordergift', (req, res) => {
  db.query("INSERT INTO orders(customer_id,total_price,advance_price,date) VALUES (?,0,0,NOW());", [req.query.cid], (err, result) => {
    res.send(result);
...
app.get('/sales_ordernotifyDeactive', (req,res) => {
  db.query("UPDATE orders SET active=0 WHERE status ='Processing' AND active=1", 
  (err, result) => {
      if (err) {
          console.log(err);
      } else {
