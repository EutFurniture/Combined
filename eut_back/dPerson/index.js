...
app.post('/addEmployee',(req,res)=>{
  ...
  transport.sendMail(mailOptions,function(error,info){
    ...
    db.query("INSERT INTO employee(name,NIC,email,phone_no,job_start_date,confirm_password,address,role,password) VALUES (?,?,?,?,?,?,?,?,?); INSERT INTO userlogin(u_email, u_name, u_password, user_role, u_otp, u_verify) VALUES (?, ?, ?, ?, ?,'0');",
    [name,NIC,email,phone_no,job_start_date,hash,address,role,hash,email,name,hash,role,otp],(err,result)=>{
...
app.get('/insertCustomizeditem', (req,res) => {
 ...
 const order_id=req.params.order_id;
  ...
  db.query("INSERT INTO orderitem(order_id,product_id) VALUES (?,?,1)",[req.query.order_id,req.query.product_id], 
...
app.get('/MaxItem',(req,res) => {
  db.query('SELECT SUM(orderitem.quantity) AS sum, products.product_name FROM orderitem INNER JOIN products ON orderitem.product_id=products.product_id GROUP BY orderitem.product_id ORDER BY sum DESC LIMIT 1', (err, result) => {
      if(err) {
          console.log(err)
      }else {
          res.send(result);
...
app.get('/MinItem',(req,res) => {
  db.query('SELECT SUM(orderitem.quantity) AS sum, products.product_name FROM orderitem INNER JOIN products ON orderitem.product_id=products.product_id GROUP BY orderitem.product_id ORDER BY sum ASC LIMIT 1', (err, result) => {
      if(err) {
          console.log(err)
      }else {
          res.send(result);
    ...
    transport.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        if (password == cpassword) {
          ...
          db.query(
            "INSERT INTO customer(fname,email,phone,address,city,postalcode,password,proimg,date,points) VALUES (?,?,?,?,?,?,?,'user.jpg',NOW(),0); INSERT INTO userlogin(u_email,u_otp,u_name,u_password,user_role) VALUES (?,?,?,?,'customer');", [fname, email, phone, address, city, post, password, email, otp, fname, password],
            (err, result) => {
              if (err) {
                console.log(err)
              } else {
    ...
    transport.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        if (password == cpassword) {
          ...
          db.query(
            "INSERT INTO customer(fname,password,email,city,phone,postalcode,address,proimg,date,points) VALUES (?,?,?,?,?,?,?,'user.jpg',NOW(),0); INSERT INTO userlogin(u_email,u_otp,u_password,u_name,user_role) VALUES (?,?,?,?,'customer');", [fname, password, email, city, phone, post, address, email, otp, password, fname],
            (err, result) => {
              if (err) {
                console.log(err)
              } else {
