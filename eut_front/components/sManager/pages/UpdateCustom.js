...
//                   <strong> Update Customer Details </strong>
//                 </Typography><br/>
...
//                 </Form.Group><br/>
//                 <div align="center">
//                  <Button  style={{fontSize:'20px',width:'200px'}} type="submit" onClick={() => {updateEmployee(Dt.customer_id)}} >Update</Button>
//                  </div> 
...
const [employeeList, setEmployeeList] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        setDt(response.data[0]);
        setNewName(response.data[0].fname)
        setNewLName(response.data[0].lname)
          setNewEmail(response.data[0].email)
          setNewPhone(response.data[0].phone)
          setNewAddress(response.data[0].address)
  ...
  const updateEmployee = (customer_id) => {
    axios.put("http://localhost:3001/sales_updateEmployee", {fname: newName,lname: newLName,email:newEmail,address:newAddress,phone:newPhone,customer_id:customer_id}).then(
      (response) => {
        
        setEmployeeList(Dt.map((val) => {
          return val.customer_id === customer_id ? {customer_id: val.customer_id, fname: val.fname, lname: val.lname, email:val.email,address:val.address,phone:val.phone,
            fname: newName,  lname: newLName, email:newEmail, address:newAddress, phone:newPhone} : val
       ...
       onChange={(event)=> {
         setNewName(event.target.value);
                ...
                <Form.Group as={Row} controlId="formHorizontalName">
                  ...
                  <Col >
                    <Form.Control type="text"  defaultValue={newLName}
       onChange={(event)=> {
